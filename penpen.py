...
def move_enemy():
    ...
    speed = 10
    if red_x % 60 == 30 and red_y % 60 == 30:       # 칸의 정확한 위치에 있는 경우 무작위로 방향변경
    ...
    if abs(red_x - pen_x) <= 40 and abs(red_y - pen_y) <= 40:       # 펜펜과 접촉했다면...
        idx = 2
        tmr = 0
...
def main():
    ...
    if idx == 5:    # 엔딩
        if tmr < 60:
            ...
            draw_txt("축하합니다!", 360, 160, 40, BLINK[tmr % 6])
        if tmr == 300:
            idx = 0
...
img_title = tkinter.PhotoImage(file="image_penpen/title.png")   # 타이틀 로고 이미지
img_ending = tkinter.PhotoImage(file="image_penpen/ending.png") # 엔딜화면
